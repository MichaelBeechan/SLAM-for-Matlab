...
%
% INPUTS: 
%   lm - set of landmarks（landmarks的坐标）
%   wp - set of waypoints（路径的坐标）
%
% OUTPUTS:
...
% initialise states
xtrue= zeros(3,1);  % 3个坐标分别是x,y和phi
x= zeros(3,1);
P= zeros(3);

% initialise other variables and constants
dt= DT_CONTROLS; % change in time between predicts(两次预测是的时间间隔)
dtsum= 0; % change in time since last observation
ftag= 1:size(lm,2); % identifier for each landmark(总共的landmark的个数)
da_table= zeros(1,size(lm,2)); % data association table 
iwp= 1; % index to first waypoint 
G= 0; % initial steer angle (G为车辆驱动方向角度，即前轮的偏角)
...
while iwp ~= 0
    % compute true data
    [G,iwp]= compute_steering(xtrue, wp, iwp, AT_WAYPOINT, G, RATEG, MAXG, dt);  % 计算到达waypoint的驱动车辆的角度
    if iwp==0 & NUMBER_LOOPS > 1, iwp=1; NUMBER_LOOPS= NUMBER_LOOPS-1; end % perform loops: if final waypoint reached, go back to first
    xtrue= vehicle_model(xtrue, V,G, WHEELBASE,dt);  % V为车辆的速度,xtrue(3)为车轴与坐标轴所成的角度
    [Vn,Gn]= add_control_noise(V,G,Q, SWITCH_CONTROL_NOISE); % 控制向量u包含速度控制V和方向控制G
    ...
    % plots
    xt= transformtoglobal(veh,xtrue);  % 车辆的实际路径
    xv= transformtoglobal(veh,x(1:3)); % 车辆的估计位置
    set(h.xt, 'xdata', xt(1,:), 'ydata', xt(2,:))
    set(h.xv, 'xdata', xv(1,:), 'ydata', xv(2,:))
    set(h.xf, 'xdata', x(4:2:end), 'ydata', x(5:2:end)) % landmark的坐标位置
...
function h= setup_animations()  % 用于设置动画的情况
